# -*- coding: utf-8 -*-
"""
Kullanıcı paneli - Userbot yönetimi
"""
...
class UserPanel:
    ...
    def __init__(self, parent, db_manager, settings, theme_manager, animation_manager,
        ...
        self.userbot_manager = UserbotManager(db_manager, settings)
    ...
    def create_header(self):
        ...
        title_label = ttk.Label(
            left_frame,
            text="🤖 Userbot Yöneticim",
            font=("Segoe UI", 16, "bold")
        )
        title_label.pack(anchor=tk.W)
        
        user_info = f"Hoş geldiniz, {self.user_data.get('username', 'Kullanıcı')} • {datetime.now().strftime('%d.%m.%Y %H:%M')}"
        ...
        # Premium durumu
        self.premium_label = ttk.Label(
            right_frame,
            text="💎 Premium",
            font=("Segoe UI", 9, "bold"),
            foreground="gold"
        )
        self.premium_label.pack(side=tk.RIGHT, padx=(0, 15))
    ...
    def create_dashboard_tab(self):
        ...
        self.total_accounts_label = self.create_stat_card(stats_frame, "🤖 Toplam Hesap", "0", 0, 0)
        self.active_accounts_label = self.create_stat_card(stats_frame, "✅ Aktif Hesap", "0", 0, 1)
        self.sent_messages_label = self.create_stat_card(stats_frame, "📤 Gönderilen Mesaj", "0", 0, 2)
        self.account_limit_label = self.create_stat_card(stats_frame, "📊 Hesap Limiti", "2", 0, 3)
        
        # Hızlı işlemler
        quick_actions_frame = ttk.LabelFrame(self.dashboard_frame, text="⚡ Hızlı İşlemler", padding=15)
        quick_actions_frame.pack(fill=tk.X, pady=(20, 0))
        
        # Butonlar
        button_frame = ttk.Frame(quick_actions_frame)
        button_frame.pack(fill=tk.X)
        
        add_account_btn = ttk.Button(
            button_frame,
        ...
        start_all_btn = ttk.Button(
            button_frame,
            text="🚀 Tümünü Başlat",
            command=self.start_all_userbots
        )
        ...
        stop_all_btn = ttk.Button(
            button_frame,
            text="🛑 Tümünü Durdur",
            command=self.stop_all_userbots
        )
        stop_all_btn.pack(side=tk.LEFT)
        
        # Son aktiviteler
        activity_frame = ttk.LabelFrame(self.dashboard_frame, text="📋 Son Aktiviteler", padding=10)
        activity_frame.pack(fill=tk.BOTH, expand=True, pady=(20, 0))
        
        # Aktivite listesi
        columns = ('Zaman', 'Hesap', 'İşlem', 'Durum')
